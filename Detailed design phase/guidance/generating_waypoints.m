...
function wp_indices = helperfunc(waypoints,states)
...
%From = [0, 0, 0, 0, 0, 0];
%To = [500, 500, 130, 0, 0, 0];

timepoints = [0]

total_distance = []
total_timepoints_vector = []
all_states = []
...
for i= 1:(size(targets, 1)-1)
    i
    startPose = trans_targets(i, :)   % [x y z qw qx qy qz]
    goalPose1 = trans_targets(i+1, :)
    ...
    initialStates = minsnappolytraj(waypoints',timepoints,nSamples,MinSegmentTime=0.1,MaxSegmentTime=20,TimeAllocation=true,TimeWeight=5000)';
    ...
    while(~valid)
        ...
        segmentIndices = helperfunc(waypoints,states);
        ...
        total_timepoints_vector = [total_timepoints_vector, timepoints];
...
plot3(states(:,1),states(:,2),states(:,3),"-y")
view([-31 63])
